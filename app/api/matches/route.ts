...
export async function POST(request: NextRequest) {
  try {
    ...
    return NextResponse.json(
      {
        error: "Une erreur est survenue lors de la création du match",
...
/**
 * GET /api/matches - Récupérer les matchs du joueur connecté
 * Query params: ?month=X&year=Y (optionnel, pour filtrer par mois)
 */
  ...
  try {
    ...
    const matches = await prisma.match.findMany({
      ...
      select: {
        ...
        player1Sets: true,
        player2Sets: true,
        set1Player1Score: true,
        set1Player2Score: true,
        set2Player1Score: true,
        set2Player2Score: true,
        set3Player1Score: true,
        set3Player2Score: true,
        matchDate: true,
        month: true,
        year: true,
        player1: {
    ...
    return NextResponse.json(
      {
        error: "Une erreur est survenue lors de la récupération des matchs",
      },
      { status: 500 }
    );
  }
